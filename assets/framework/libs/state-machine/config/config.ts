...
import {
  DefaultConfig, HardTransition, LifeCycle,
  Methods,
  Options, TransitionLookup
} from "./config.types";
...
class Config {
  ...
  public transitionFor(state: State, transition: string): HardTransition | Record<string, unknown> {
    ...
    let r = this.map[state][transition] || this.map[wildcard][transition];
    var regFunc = /[(:|=|{|})+*\.]/g;
    let regexpResult;
    if (!r) {
      for (let k in this.map) {
        let v = this.map[k];
        if (k != wildcard && regFunc.test(k)) {
          if (new RegExp(k).test(state)) {
            regexpResult = this.map[k][transition];
            break;
          }
        }
      }
    }
    return  r  || regexpResult;
  }
